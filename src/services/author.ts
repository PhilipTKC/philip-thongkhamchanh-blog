...
export const IAuthorService = DI.createInterface<IAuthorService>(
  "IAuthorService",
  (x) => x.singleton(AuthorService)
);

export class AuthorService {
  async retrieveAuthor(author: string): Promise<Author> {
    try {
      const contributors = await import(
  ...
  async retrieveAuthorPosts(author: string, page: number): Promise<any> {
    try {
      const authorData = await import(
        `../../content/authors-data/${author}.json`
      ).then(({ default: _ }) => _);
      const authorPosts = await import(
        `../../content/authors-data/${author}-${page}.json`
      ).then(({ default: _ }) => _);
