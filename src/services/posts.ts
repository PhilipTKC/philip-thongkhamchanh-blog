...
export const IPostService = DI.createInterface<IPostService>(
  "IPostService",
  (x) => x.singleton(PostService)
);
  ...
  async retrievePost(entry: PostData | string): Promise<any> {
    try {
      if (typeof entry === "string") {
        const post = await import(`../../content/posts/${entry}.md`).then(
          ({ default: _ }) => _
        );
        return post;
      }
